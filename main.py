...
setTitle("Server Booster - [RedCrystal Inviter]")
  ...
  async with aiohttp.ClientSession(headers=headers) as serverjoinersession:
    async with serverjoinersession.post(f"https://discord.com/api/v9/invites/{inv}") as response:
      if response.status in (204, 200, 201):
        print(f"[-] RedCrystal | Successfully Joined Server")
      else:
        print(f"[-] RedCrystal | Failed To Join Server, Status Code: {response.status}")
        ...
        for varr in idk_var:
          ...
          async with ClientSession.put(f"https://discord.com/api/v9/guilds/{guildid}/premium/subscriptions", json=payload) as boost_req:
            ...
            if "id" in btxt:
              ...
              print(f"[-] RedCrystal | Successfully Boosted Server")
            else:
              print("[-] RedCrystal | Failed To Boost Server, Unknown Error Occurred")


banner = f"""{Fore.RED}[-]{Fore.RESET} Created By Rloxx\n\n{Fore.BLUE}[1]{Fore.RESET} Server Joiner\n\n{Fore.BLUE}[2]{Fore.RESET} Boost Server\n"""
...
print(banner)
ch = input("[-] RedCrystal | Choice: ")
try:
  c = int(ch)
except ValueError:
  print("RedCrystal | Use Number To Choose.")
  sys.exit()
  
if c == 1:
  invv = input("[-] RedCrystal | Enter Invite Code: discord.gg/")
  asyncio.run(start_join(invv))
elif c == 2:
  g = int(input("[-] RedCrystal | Enter Guild ID: "))
  asyncio.run(start_boost(g))
else:
  print("[-] RedCrystal | Invaild Option")
  exit(0)
